...
import type { PillarArea, PrimePillar } from '@shared/types/models';
...
export function PillarCard({ pillar, className }: PillarCardProps) {
  ...
  const config = PILLAR_MAP[pillar.area];
  const PillarIcon = PILLAR_ICONS[pillar.area];
  const outcomeCount = pillar.outcomes?.length ?? 0;

  const averageProgress =
    outcomeCount > 0
      ? Math.round(
          pillar.outcomes.reduce((sum, o) => sum + (o.progress ?? 0), 0) / outcomeCount
        )
      : 0;

  const handlePress = () => {
    router.push(`/(app)/(tabs)/goals/pillar/${pillar.id}`);
  };
