...
import { usePillars } from '../hooks/useGoals';
...
interface PillarPickerSheetProps {
  isOpen: boolean;
  onClose: () => void;
  selectedPillarId?: string | null;
  onSelect: (pillar: PrimePillar | null) => void;
}

export function PillarPickerSheet({
  isOpen,
  onClose,
  selectedPillarId,
  onSelect,
}: PillarPickerSheetProps) {
  ...
  return (
    ...
    >
      <View className="px-4 pb-3">
        <Text className="text-center text-lg font-semibold text-foreground">Link to Pillar</Text>
        <Text className="mt-1 text-center text-xs text-muted-foreground">
          Select a Prime Roadmap pillar to link this habit to
        </Text>
      </View>
      ...
      <BottomSheetScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: 60 }}>
        {isLoading ? (
          <View className="py-10 items-center gap-2">
            ...
            <Text className="text-sm text-muted-foreground">Loading pillars...</Text>
          </View>
        ) : pillars.length === 0 ? (
          <View className="py-10 items-center gap-2">
            <Icon as={Layers} size={32} className="text-muted-foreground" />
            <Text className="text-sm font-medium text-foreground">No pillars yet</Text>
            <Text className="text-xs text-center text-muted-foreground">
              Create your Prime Roadmap pillars in the Goals section first
            </Text>
          </View>
        ) : (
          <View className="gap-2">
            ...
            >
              ...
              <Text className="flex-1 text-sm text-muted-foreground italic">No pillar linked</Text>
              {!selectedPillarId && (
            ...
            {pillars.map((pillar) => {
              ...
              const areaColor = PILLAR_AREA_COLORS[pillar.area] ?? '#6366f1';
              const areaLabel = PILLAR_AREA_LABELS[pillar.area] ?? pillar.area;
