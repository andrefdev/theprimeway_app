import { cn } from '@/shared/utils/cn';
import { Text } from '@/shared/components/ui/text';
import { Icon } from '@/shared/components/ui/icon';
import { ChevronRight, Calendar } from 'lucide-react-native';
import { Pressable, View } from 'react-native';
import { router } from 'expo-router';
import { useTranslation } from '@/shared/hooks/useTranslation';
import type { PrimeOutcome } from '@shared/types/models';

interface OutcomeItemProps {
  outcome: PrimeOutcome;
  color?: string;
  className?: string;
}

export function OutcomeItem({ outcome, color = '#3b82f6', className }: OutcomeItemProps) {
  const { locale } = useTranslation();
  const handlePress = () => {
    router.push(`/(app)/(tabs)/goals/outcome/${outcome.id}`);
  };

  const formattedDate = outcome.targetDate
    ? new Date(outcome.targetDate).toLocaleDateString(locale, {
        month: 'short',
        year: 'numeric',
      })
    : null;

  return (
    <Pressable
      onPress={handlePress}
      className={cn(
        'flex-row items-center rounded-xl border border-border bg-card p-4 active:opacity-80',
        className
      )}
    >
      {/* Color Indicator */}
      <View className="mr-3 h-10 w-1 rounded-full" style={{ backgroundColor: color }} />

      {/* Content */}
      <View className="flex-1">
        <Text className="text-sm font-semibold text-card-foreground" numberOfLines={1}>
          {outcome.title}
        </Text>

        {/* Progress + Date Row */}
        <View className="mt-2 flex-row items-center gap-3">
          {/* Progress Bar */}
          <View className="h-1.5 flex-1 overflow-hidden rounded-full bg-muted">
            <View
              className="h-full rounded-full"
              style={{
                backgroundColor: color,
                width: `${outcome.progress ?? 0}%`,
              }}
            />
          </View>
          <Text className="text-xs font-medium text-muted-foreground">
            {outcome.progress ?? 0}%
          </Text>
        </View>

        {/* Target Date */}
        {formattedDate && (
          <View className="mt-1.5 flex-row items-center gap-1">
            <Icon as={Calendar} size={12} className="text-muted-foreground" />
            <Text className="text-xs text-muted-foreground">{formattedDate}</Text>
          </View>
        )}
      </View>

      {/* Chevron */}
      <Icon as={ChevronRight} size={16} className="ml-2 text-muted-foreground" />
    </Pressable>
  );
}
