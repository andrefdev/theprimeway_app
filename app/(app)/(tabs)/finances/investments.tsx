...
export default function InvestmentsScreen() {
  ...
  const renderInvestment = ({ item, index }: { item: Investment; index: number }) => {
    ...
    const typeLabel = INVESTMENT_TYPE_LABELS[item.investmentType] ?? item.investmentType;
        ...
        <View className="mx-4 mb-3 rounded-xl border border-border bg-card p-4">
          ...
          <View className="mt-3 flex-row items-end justify-between">
            ...
            <View className="flex-1 items-end">
              <Text className="text-xs text-muted-foreground">Gain</Text>
          ...
          {item.purchaseDate && (
            <View className="mt-3 border-t border-border pt-2">
              <Text className="text-xs text-muted-foreground">
                Since {formatDate(item.purchaseDate, 'MMM d, yyyy')}
              </Text>
            </View>
          )}
        </View>
        ...
        ListHeaderComponent={
          <Animated.View entering={FadeInDown.springify()} className="items-center px-4 pb-5 pt-4">
            ...
            <Text className="mt-1 text-xs text-muted-foreground">
              {formatCurrency(summary?.totalInvested ?? 0)} invested · {investments.length}{' '}
