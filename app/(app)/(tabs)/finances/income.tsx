...
export default function IncomeScreen() {
    ...
    return (
      <View className="mx-4 mb-3 rounded-xl border border-border bg-card p-4">
        <View className="flex-row items-start justify-between">
          ...
          <View className="items-end gap-1">
            ...
            >
              ...
              >
                {item.isActive ? t('income.active') : t('income.inactive')}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View className="mt-3 flex-row items-end justify-between">
          <View>
            <Text className="text-xs text-muted-foreground">{t('income.amountPerFrequency')}</Text>
            <Text className="mt-0.5 text-base font-bold text-card-foreground">
              {formatCurrency(item.amount, item.currency)}
            </Text>
          </View>
          <View className="items-end">
            <Text className="text-xs text-muted-foreground">{t('income.monthlyEquivalent')}</Text>
  ...
  if (!sources || sources.length === 0) {
    return (
      <EmptyState
        icon={TrendingUp}
        title={t('income.messages.noSourcesYet')}
        description={t('income.messages.addFirst')}
      />
    );
  }
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View className="items-center px-4 pb-5 pt-4">
            <Text className="text-xs text-muted-foreground">{t('income.totalMonthly')}</Text>
            <Text className="mt-1 text-3xl font-bold text-emerald-600">
              {formatCurrency(totalMonthly)}
            </Text>
            <Text className="mt-1 text-xs text-muted-foreground">
              {sources.filter((s) => s.isActive).length} {t('income.activeSources')}
            </Text>
          </View>
        }
      />
