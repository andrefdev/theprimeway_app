...
export default function GoalsScreen() {
  ...
  return (
    <Screen>
      ...
      {activeTab === 'roadmap' && (
        <RoadmapTab visions={visions} pillars={pillars} t={t} />
      )}
...
function RoadmapTab({
  ...
  const hasData = !!vision || (pillars && pillars.length > 0);
  const showExample = !hasData && !exampleDismissed;

  const pillarsByArea = new Map(
    (pillars ?? []).map((p: any) => [p.area, p])
  );
      ...
      <ScreenContent>
        ...
        <View className="flex-row flex-wrap gap-3">
          {pillarGrid.map((pillar) => (
            <PillarCard key={pillar.id} pillar={pillar} className="w-[48%] flex-grow" />
          ))}
        </View>
      </ScreenContent>
    </ScrollView>
    ...
    return (
      <View className="flex-1 px-4 pt-8">
        <EmptyState
          ...
          description="Create outcomes under your pillars to define what you want to achieve."
        />
      </View>
    );
  }
  ...
  return (
    ...
    >
      <View className="gap-3">
        {(outcomes as any[]).map((outcome, index) => {
          const pillar = pillarMap.get(outcome.pillarId);
          return (
            <Animated.View key={outcome.id} entering={FadeInDown.delay(index * 50).duration(300)}>
              <View className="rounded-xl border border-border bg-card p-4">
                ...
                <Text className="text-sm font-semibold text-foreground">{outcome.title}</Text>
                {outcome.description && (
                ...
                {outcome.deadline && (
                  <Text className="mt-1.5 text-xs text-muted-foreground">
                    Deadline: {new Date(outcome.deadline).toLocaleDateString()}
                  </Text>
                )}

                {/* Focuses under this outcome */}
                {outcome.focuses && outcome.focuses.length > 0 && (
                  <View className="mt-3 gap-2 border-t border-border pt-3">
                    {outcome.focuses.map((focus: any) => (
                      <View key={focus.id} className="flex-row items-center gap-2">
                        <Icon
                          as={focus.status === 'completed' ? CheckCircle2 : ChevronRight}
                          size={14}
                          className={focus.status === 'completed' ? 'text-green-500' : 'text-muted-foreground'}
                        />
                        <Text
                        ...
                        {focus.progress != null && (
                          <Text className="text-xs text-muted-foreground">{focus.progress}%</Text>
                        )}
                      </View>
                    ))}
                  </View>
...
function QuarterlyTab({ t }: { t: (k: string) => string }) {
  ...
  const activeFocuses = (allFocuses as any[]).filter((f) => f.status !== 'completed');
  const completedFocuses = (allFocuses as any[]).filter((f) => f.status === 'completed');
    ...
    >
      ...
      <Animated.View entering={FadeInDown.delay(50).duration(300)}>
        <Text className="mb-2 text-sm font-semibold text-foreground">
          Active Focuses ({activeFocuses.length})
        </Text>
        {activeFocuses.length === 0 ? (
          <EmptyState
            icon={Crosshair}
            title="No active focuses"
            description="Add focuses under your outcomes to track quarterly progress."
          />
        ) : (
          <View className="gap-3">
            {activeFocuses.map((focus: any) => (
              <View key={focus.id} className="rounded-xl border border-border bg-card p-4">
                <Text className="text-sm font-semibold text-foreground">{focus.title}</Text>
                {focus.description && (
                  <Text className="mt-0.5 text-xs text-muted-foreground" numberOfLines={2}>
                    {focus.description}
                  </Text>
                ...
                <View className="mt-3">
                  ...
                  <View className="mt-1 h-2 overflow-hidden rounded-full bg-muted">
                    <View
                      ...
                      style={{ width: `${Math.min(focus.progress ?? 0, 100)}%` }}
                    />
                  </View>
                </View>

                {/* Key results */}
                {focus.keyResults && focus.keyResults.length > 0 && (
                  <View className="mt-3 gap-1.5 border-t border-border pt-3">
                    ...
                    {focus.keyResults.map((kr: any, idx: number) => (
                ...
                {/* Focus Links (Part 3) */}
                <FocusLinksRow focusId={focus.id} />
              </View>
            ))}
          </View>
        )}
      ...
      {completedFocuses.length > 0 && (
        <Animated.View entering={FadeInDown.delay(100).duration(300)} className="mt-6">
          <Text className="mb-2 text-sm font-semibold text-foreground">
            Completed ({completedFocuses.length})
          </Text>
          <View className="gap-2">
            {completedFocuses.slice(0, 5).map((focus: any) => (
              <View key={focus.id} className="flex-row items-center gap-2 rounded-xl border border-border bg-card/50 p-3">
