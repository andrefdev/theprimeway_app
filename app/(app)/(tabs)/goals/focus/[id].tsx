...
import { useFocuses, useWeeklyGoals, useCreateWeeklyGoal } from '@/features/goals/hooks/useGoals';
import {
  Plus,
  Calendar,
  CheckCircle2,
...
export default function FocusDetailScreen() {
  ...
  const { data: focuses, isLoading: focusesLoading, isError: focusesError, refetch: refetchFocuses } = useFocuses();
  ...
  const isLoading = focusesLoading || weeklyLoading;
  const isError = focusesError || weeklyError;
  ...
  const handleCreateWeeklyGoal = async () => {
    ...
    await createWeeklyGoal.mutateAsync({
      ...
      order: (allWeeklyGoals ?? []).filter((wg) => wg.quarterFocusId === id).length,
    });
    setGoalTitle('');
    setGoalDescription('');
    setShowForm(false);
  ...
  const weeklyGoals = (allWeeklyGoals ?? [])
    .filter((wg) => wg.quarterFocusId === id)
    .sort((a, b) => a.order - b.order);

  const formattedPeriod =
    focus?.startDate && focus?.endDate
      ? `${new Date(focus.startDate).toLocaleDateString(locale, {
          month: 'short',
          day: 'numeric',
        })} - ${new Date(focus.endDate).toLocaleDateString(locale, {
          month: 'short',
          day: 'numeric',
          year: 'numeric',
        })}`
  ...
  if (isLoading) {
    return (
      <Screen>
        <Header title={t('focus.title')} showBack />
  ...
  return (
    <Screen>
      <Header
        title={`Q${focus.quarter} ${focus.year}`}
        showBack
        rightAction={
          ...
          <View className="mb-5 rounded-xl border border-border bg-card p-5">
            <View className="mb-3 flex-row items-center gap-3">
              ...
              <View className="flex-1">
                <Text className="text-lg font-bold text-card-foreground">{focus.title}</Text>
                <View className="mt-0.5 flex-row items-center gap-1">
                  <Text className="text-xs font-medium text-primary">
                    Q{focus.quarter} {focus.year}
                  </Text>
                  {formattedPeriod && (
          ...
          <View className="flex-1 px-4 gap-5 pt-4">
            ...
            >
              {createWeeklyGoal.isPending ? (
                ...
                <Text className="text-sm font-medium text-primary-foreground">{t('actions.create')}</Text>
              )}
            </Pressable>
          </View>
        </SafeAreaView>
